...
def config_loader(path="config.yaml"):
    ...
    try:
        ...
        if config_data is None:
            print(f"File '{path}' is empty or doen't contain valid YAML")
            return None
        
        # Validar estructura: debe contener seccion 'cluster'
        if 'cluster' not in config_data:
            print(f"Incomplete configuration: missing 'cluster' section")
            return None
        
        # Validar estructura: debe contener seccion 'svm'
        if 'svm' not in config_data:
            print(f"Incomplete configuration: missing 'svm' section")
            return None
...
# Establish connection using configuration data
config.CONNECTION = HostConnection(
    config_data['cluster']['host'],
    username=config_data['cluster']['username'],
    password=config_data['cluster']['password'],
    verify=False
)
